...
const { parseBlufFindings, extractAgentOutputs } = require('./lib/bluf-parser');
const { saveFinding } = require('./lib/agent-findings');
const { recordSessionMetrics } = require('./lib/metrics-tracker');
...
async function main() {
  ...
  try {
    ...
    if (entries.length > 0) {
      ...
      client.pruneDecayed(containerTag);
      ...
      // Extract and save agent findings (Shared Agent Memory)
      const agentOutputs = extractAgentOutputs(entries);
      let findingsCount = 0;
      for (const { agentName, output } of agentOutputs) {
      ...
      for (const { agentName, output } of agentOutputs) {
        ...
        for (const f of parseBlufFindings(output)) {
          severityCounts[f.severity] = (severityCounts[f.severity] || 0) + 1;
        }
      }
      try {
        recordSessionMetrics(getDb(), containerTag, sessionId, {
          agentsSpawned: agentOutputs.length,
          findingsCaptured: findingsCount,
          findingsBySeverity: severityCounts,
          agentsUsed: [...agentNames],
        });
      } catch {
        // Metrics are non-critical — don't fail the hook
      }

      debugLog(settings, 'Facts extracted', {
        session: facts.sessionFacts.length,
        user: facts.userFacts.length,
        agentFindings: findingsCount,
      });
    }
